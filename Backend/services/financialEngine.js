...
const getFinancialAnalytics = async (userId) => {
  ...
  try {
    ...
    // Fetch all transactions for the user ordered by created_at ASC
    ...
    const monthsWithExpenses = monthlyDataArray.filter(month => month.expense > 0);
    const averageMonthlyExpense = monthsWithExpenses.length > 0 
      ? totalExpense / monthsWithExpenses.length 
      : 0;
    
    // Calculate month-to-month growth rate of expenses
    let growthRate = 0;
    if (monthlyDataArray.length >= 2) {
      ...
      for (let i = 1; i < expenses.length; i++) {
        if (expenses[i - 1] > 0) {
          const growth = ((expenses[i] - expenses[i - 1]) / expenses[i - 1]) * 100;
          totalGrowth += growth;
          growthPeriods++;
        }
      }
      
      growthRate = growthPeriods > 0 ? totalGrowth / growthPeriods : 0;
    }
    ...
    for (const [category, total] of Object.entries(categoryTotals)) {
      roundedCategoryTotals[category] = Math.round(total * 100) / 100;
    }
